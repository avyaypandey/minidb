...
TEST(SerializerTest, SerializesDifferentStringLengths)
{
    ...
    tuple.AddString("this is a much longer string value");
...
TEST(SerializerTest, RejectsTruncatedTuple)
{
    ...
    EXPECT_THROW(Serializer::Deserialize(bytes), std::runtime_error);
}

TEST(SerializerTest, RejectsTruncatedString)
{
    std::vector<std::byte> bytes = {std::byte{0x01}, // field count low byte
