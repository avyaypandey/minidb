...
TEST(PageTest, InitializesFullFreeSpace)
{
    ...
    EXPECT_EQ(page.FreeSpace(), PAGE_SIZE - 4);
}
...
TEST(PageTest, InsertsSingleRecord)
{
    ...
    std::vector<std::byte> record = {std::byte{0x01}, std::byte{0x02}, std::byte{0x03},
                                     std::byte{0x04}};

    auto rid = page.InsertRecord(record.data(), record.size());
...
TEST(PageTest, RetrievesMultipleRecords)
{
    ...
    std::vector<std::byte> first = {std::byte{0xAA}, std::byte{0xBB}};

    std::vector<std::byte> second = {std::byte{0x10}, std::byte{0x20}, std::byte{0x30},
                                     std::byte{0x40}};

    std::vector<std::byte> third = {std::byte{0xFF}};

    auto first_rid = page.InsertRecord(first.data(), first.size());

    auto second_rid = page.InsertRecord(second.data(), second.size());

    auto third_rid = page.InsertRecord(third.data(), third.size());
...
TEST(PageTest, DeleteDoesNotAffectOtherRecords)
{
    ...
    std::vector<std::byte> first = {std::byte{0x10}};

    std::vector<std::byte> second = {std::byte{0x20}, std::byte{0x30}};

    std::vector<std::byte> third = {std::byte{0x40}, std::byte{0x50}, std::byte{0x60}};
