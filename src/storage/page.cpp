 ...
 */
std::optional<RID> Page::InsertRecord(const std::byte* record, std::size_t size)
{
    uint16_t num_slots = NumSlots();
    uint16_t free_offset = FreeSpaceOffset();

    std::size_t slot_directory_end = HEADER_SIZE + (num_slots * SLOT_SIZE);
    ...
    uint16_t new_free_offset = static_cast<uint16_t>(free_offset - size);

    if (new_free_offset < slot_directory_end + SLOT_SIZE)
    {
        return std::nullopt;
    }

    // Copy record bytes into page
    std::memcpy(data_.data() + new_free_offset, record, size);
...
std::optional<std::vector<std::byte>> Page::GetRecord(RID rid) const
{
    ...
    std::memcpy(record.data(), data_.data() + slot.offset, slot.length);
 ...
 */
uint16_t Page::NumSlots() const { return ReadUInt16(NUM_SLOTS_OFFSET); }

/**
 * @brief Returns the current byte offset where free space begins.
 */
uint16_t Page::FreeSpaceOffset() const { return ReadUInt16(FREE_SPACE_OFFSET); }
 ...
 */
void Page::SetNumSlots(uint16_t value) { WriteUInt16(NUM_SLOTS_OFFSET, value); }

/**
 * @brief Updates the free space offset pointer in the page header.
 */
void Page::SetFreeSpaceOffset(uint16_t value) { WriteUInt16(FREE_SPACE_OFFSET, value); }
